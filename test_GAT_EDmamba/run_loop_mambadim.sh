...
for DATASET in "${DATASETS[@]}"; do
  for mamba_hidden in "${mamba_hidden_dim[@]}"; do
    echo "Training for DATASET=${DATASET}, mamba_hidden_dim=${mamba_hidden}"
    ...
    PYTHONPATH="$PROJECT_ROOT" python test_GAT_EDmamba/train_model.py \
      ...
      --seed-list "${SEEDS[@]}" \
      --model-code "$MODEL_CODE"_"$mamba_hidden" \
      --asym-alpha "$alpha" \
      --asym-gamma "$gamma" \
      --asym-delta "$delta" \
      --focus-threshold "$focus_threshold" \
      --cap-threshold "$cap_threshold" \
      --mamba-hidden-dim "$mamba_hidden"
  done
done
