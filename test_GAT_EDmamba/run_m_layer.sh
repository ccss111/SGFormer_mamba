...
for DATASET in "${DATASETS[@]}"; do
  for mamba_layer in "${mamba_layers[@]}"; do
    echo "Training for DATASET=${DATASET}, mamba_layer=${mamba_layer}"
    ...
    PYTHONPATH="$PROJECT_ROOT" python test_GAT_EDmamba/train_model.py \
      ...
      --seed-list "${SEEDS[@]}" \
      --model-code "$MODEL_CODE"_"$mamba_layer" \
      --asym-alpha "$alpha" \
      --asym-gamma "$gamma" \
      --asym-delta "$delta" \
      --focus-threshold "$focus_threshold" \
      --cap-threshold "$cap_threshold" \
      --mamba-num-layers "$mamba_layer" \
